...
class OAuthClient():
  ...
  def make_async_request(self, url, token="", secret="", additional_params=None,
    ...
    logging.info("oAuth: making request (%s, %s, %s)" % (url, method, payload))
...
"""
FB graph model class

@TODO:  IMPLEMENTZZZZOOOOOORRRRR!!!!!!!
@TODO:  https://github.com/facebook/python-sdk/raw/master/examples/oauth/facebookoauth.py
@TODO:  http://developers.facebook.com/docs/authentication/
@TODO:  http://www.facebook.com/developers/apps.php?app_id=188275614546477&ret=2
"""
class FacebookClient(OAuthClient):
  ...
  def get_authorization_url(self):
    ...
    return "http://www.facebook.com/dialog/oauth?client_id=%s&redirect_uri=%s" % token
...
class TwitterClient(OAuthClient):
  ...
  def get_friends(self, access_token, access_secret):
    response = self.make_request(
      "http://api.twitter.com/1/account/settings.json",
      token=access_token, secret=access_secret, protected=True)
    
    data = json.loads(response.content)

    logging.info("GOT USER FRIENDS:")
    logging.info(data)
    return data
  ...
  def _lookup_user_info(self, access_token, access_secret):
    ...
    user_info["name"] = data["name"]
    user_info["picture"] = data["profile_image_url"]
    user_info["_token"] = access_token
    user_info["_secret"] = access_secret
